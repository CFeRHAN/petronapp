...
@api_view(['GET'])
def flow(request, offer_pk):
    user = request.user

    try:
        offer = Offer.objects.get(pk=offer_pk)
    except Offer.DoesNotExist:
        return Response({'message': f'offer {offer_pk} not found'}, status=status.HTTP_404_NOT_FOUND)


    def _trader_():
        print('something')
        return Response({'message': f'this {offer_pk}'}, status=status.HTTP_200_OK)
    ...
    if offer.order.contract_type == '1':
        print(offer.order.contract_type)
